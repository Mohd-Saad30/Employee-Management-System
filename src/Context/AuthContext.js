...
export const AuthStore = create(persist(
    (set, get) => ({
        ...
        login: (email, password) => {
            ...
            const adminUser = adminData.find(admin => admin.email == email && admin.password == password);
            if (adminUser) {
                set({ user: { role: "admin", ...adminUser } })
                return true
            }
            const employeeUser = employeesData.find(employee => employee.email == email && employee.password == password)
            if (employeeUser) {
        ...
        addTask: (employeeName, Task) => set((state) => ({
